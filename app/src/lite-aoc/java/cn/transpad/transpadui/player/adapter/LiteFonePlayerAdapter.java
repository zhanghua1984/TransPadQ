 ...
 * @since 2014-4-18
 */
public abstract class LiteFonePlayerAdapter implements IPlayerAdapter {
    /**
     ...
     * log Tag
     */
    private static final String TAG = "LiteFonePlayerAdapter";
    /**
     * 视频播放类
     */
    ...
    public LiteFonePlayerAdapter(SurfaceView videoSurface) {
        super();
    ...
    public void dlnaOpen(final DLNADevice device) {
        stop();
        release();
        submitRunable(new Runnable() {
            @Override
            public void run() {
                int pos = currentPostion == 0 ? currentMedia.seekPos / 1000 : currentPostion / 1000;
                boolean result = DLNAPlayer.getInstance(TransPadApplication.getTransPadApplication()).open(device.server_uid, currentMedia.fragUrl[0], "", "", pos);
                L.v(TAG, "dlna open result = " + result);
                if (result) {
                    isDlnaMode = true;
                    currentDevice = device;
                    videoDuration = 0;
                    dlnaOpenStateThread = new Thread(dlnaOpenStateRunnable);
                    dlnaOpenStateThread.start();

                    dlnaPositionThread = new Thread(dlnaPositionRunnable);
                    dlnaPositionThread.start();
                } else {
                    sendMessage2UI(FONE_PLAYER_MSG_DLNA_OPEN_FAILED);
                }
            }
        });
        sendMessage2UI(FONE_PLAYER_MSG_DLNA_OPENING);
    }

    public void dlnaRetry() {
        if (currentDevice != null) {
            dlnaOpen(currentDevice);
        }
    }

    private Thread dlnaOpenStateThread;

    private Runnable dlnaOpenStateRunnable = new Runnable() {
        ...
        public void run() {
            try {
                playingTimes = 0;
                stopTimes = 0;
                while (playingTimes < 30 && stopTimes < 30) {
                    if (currentDevice != null) {
                        int state = DLNAPlayer.getInstance(TransPadApplication.getTransPadApplication()).getPlayState(currentDevice.server_uid);
                        L.v(TAG, "dlnaOpenStateRunnable state = " + state);
                        if (state == DLNAPlayer.PLAYSTATE_PLAYING) {
                            playingTimes++;
                        } else if (state == DLNAPlayer.PLAYSTATE_STOPPED || state == DLNAPlayer.PLAYSTATE_UNKNOWN) {
                            stopTimes++;
                        }
                    }
                    Thread.sleep(500);
                ...
                if (playingTimes >= 30) {
                    isPlaying = true;
                    sendMessage2UI(FONE_PLAYER_MSG_DLNA_OPEN_SUCCED);
                }
                if (stopTimes >= 30) {
                    isPlaying = false;
                    sendMessage2UI(FONE_PLAYER_MSG_DLNA_OPEN_FAILED);
    ...
    private Thread dlnaPositionThread;

    private Runnable dlnaPositionRunnable = new Runnable() {
            ...
            while (isDlnaMode) {
                try {
                    if (isPlaying && currentDevice != null) {
                        if (videoDuration <= 0) {
                            videoDuration = (int) DLNAPlayer.getInstance(TransPadApplication.getTransPadApplication()).getDuration(currentDevice.server_uid) * 1000;
                            L.v(TAG, "dlna videoDuration = " + videoDuration);
                        }
                        int tempPosition = (int) DLNAPlayer.getInstance(TransPadApplication.getTransPadApplication()).getCurrentTime(currentDevice.server_uid) * 1000;
                        if (tempPosition > 0) {
                            currentPostion = tempPosition;
                            errorTimes = 0;
                        }else {
                            errorTimes ++;
                            if (errorTimes >= 30) {
                                //设备断开
                                sendMessage2UI(FONE_PLAYER_MSG_DLNA_DEVICE_DISCONNECT);
                                break;
                            }
                        }
                        L.v(TAG, "dlna currentPostion = " + currentPostion);
                    }
                    if (currentDevice != null) {
                        int state = DLNAPlayer.getInstance(TransPadApplication.getTransPadApplication()).getPlayState(currentDevice.server_uid);
                        L.v(TAG, "dlna state = " + state);
                        if (state == DLNAPlayer.PLAYSTATE_STOPPED || state == DLNAPlayer.PLAYSTATE_UNKNOWN) {
                            isPlaying = false;
                            if (state == DLNAPlayer.PLAYSTATE_STOPPED && videoDuration > 0 && currentPostion >= videoDuration - 5000) {
                                sendMessage2UI(FONE_PLAYER_MSG_DLNA_PLAY_COMPLETION);
                                break;
                            }
                        }else if (state == DLNAPlayer.PLAYSTATE_PLAYING || state == DLNAPlayer.PLAYSTATE_TRANSITIONING){
                            isPlaying = true;
                        }
                    }
                    Thread.sleep(500);
     ...
     * @param device
     */
    public void dlnaStop(final DLNADevice device) {
        submitRunable(new Runnable() {
            @Override
            public void run() {
                DLNAPlayer.getInstance(TransPadApplication.getTransPadApplication()).stop(device.server_uid);
            }
        });
        savePlayRecord();
    ...
    private class FoneOnShowEngineTypeLister implements OnShowEngineTypeListener {
        public void onEngineType(FonePlayer fp, int engineType) {
            ...
            LiteFonePlayerAdapter.this.engineType = engineType;
    ...
    public void pause() {
        ...
        if (isDlnaMode && currentDevice != null) {
            submitRunable(new Runnable() {
                @Override
                public void run() {
                    boolean result = DLNAPlayer.getInstance(TransPadApplication.getTransPadApplication()).pause(currentDevice.server_uid);
                    if (result) {
                        isPlaying = false;
    ...
    public synchronized void stop() {
        ...
        if (isDlnaMode && currentDevice != null) {
            submitRunable(new Runnable() {
                @Override
                public void run() {
                    DLNAPlayer.getInstance(TransPadApplication.getTransPadApplication()).stop(currentDevice.server_uid);
                }
            });
        } else {
            if (player != null) {
    ...
    public void seekTo(final int progress) {
        ...
        if (isDlnaMode && currentDevice != null) {
            submitRunable(new Runnable() {
                @Override
                public void run() {
                    L.v(TAG, "dlna seekTo = " + progress);
                    DLNAPlayer.getInstance(TransPadApplication.getTransPadApplication()).seek(currentDevice.server_uid, progress / 1000);
                }
            });
        } else {
    ...
    public synchronized void play() {
        ...
        if (isDlnaMode && currentDevice != null) {
            new Thread(new Runnable() {
                ...
                public void run() {
                    int state = DLNAPlayer.getInstance(TransPadApplication.getTransPadApplication()).getPlayState(currentDevice.server_uid);
                    L.v(TAG,"play state = " + state);
                    if (state == DLNAPlayer.PLAYSTATE_PAUSED_PLAYBACK || state == DLNAPlayer.PLAYSTATE_PLAYING) {
                        boolean result = DLNAPlayer.getInstance(TransPadApplication.getTransPadApplication()).resume(currentDevice.server_uid);
                        if (result) {
                            isPlaying = true;
                            sendMessage2UI(FONE_PLAYER_MSG_PLAYER_PLAYING_PAUSE);
                        }
...
//        updateCurrentMedia(multipleVideos.get(index).getUrls(), new int[]{0}, 0, 0);
    ...
    public String getDlnaDeviceName() {
        if (currentDevice != null) {
            return currentDevice.server_name;
        }
        return "";
    }
