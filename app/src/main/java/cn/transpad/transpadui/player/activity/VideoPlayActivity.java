...
//        hideNavigation();
    ...
    private void initPlayerAdapter() {
        playerAdapter = new FonePlayerAdapter(playerView) {
            @Override
                ...
                try {
                    if (null != uriPath) {
                        ...
                        if (TextUtils.isEmpty(url)) {
                            Toast.makeText(VideoPlayActivity.this, R.string.play_url_null, Toast.LENGTH_SHORT).show();
                            finish();
                        } else {
    ...
    private void initPlayerGestureListener() {
        ...
        foneOnGesture.setFoneOnGestureListener(new FoneOnGesture.FoneOnGestureListener() {
            ...
            public void FoneOnGestureMoveUPOrDown(boolean isEnableSeek, float distance, float start_x, boolean two_pointer) {
                ...
                if (width == 0) {
                    width = ScreenUtil.getScreenWidthPix(VideoPlayActivity.this);
                }
                ...
                if (status == 1) {
                    ...
                    // 修改音量
                    int addVol = (int) (distance / (ScreenUtil.getScreenHeightPix(VideoPlayActivity.this) / 50));
                    ...
                    // 修改亮度
                    float addBright = distance / (float) (ScreenUtil.getScreenHeightPix(VideoPlayActivity.this) * 10);
...
//        }
    ...
    private void setSurfaceWidthHeight(int width, int height) {
        ...
        int leftRight = (ScreenUtil.getScreenWidthPix(VideoPlayActivity.this) - width) / 2;
        int topBottom = 0;
        topBottom = (ScreenUtil.getScreenHeightPix(VideoPlayActivity.this) - height) / 2;
        if (topBottom <= 0) {
            topBottom = 1;
        }
