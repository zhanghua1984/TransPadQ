...
public class NumberCircleProgressBar extends View {
    private static final String TAG = NumberCircleProgressBar.class.getSimpleName();
    ...
    public NumberCircleProgressBar(Context context, AttributeSet attrs,
        ...
        try {
            ...
            Log.i("NumberCircleProgressBar", "setProgress = " + getProgress());
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
//		L.i(TAG,"onDraw","max = " + getMax() + "progress = " + getProgress());
        ...
        if (mDrawReachedBar) {
            switch (mFillMode) {
                case 0:
//                L.i(TAG,"onDraw","onDraw");
                    canvas.drawArc(mCircleRectF, getProgress() * 360 / getMax() + DEFAULT_INITIAL_ANGLE,
                            360 - getProgress() * 360 / getMax(), true, mSectorPaint);
                    break;
                case 1:
...
////        L.i(TAG,"onSaveInstanceState","bundle="+bundle);
//		return bundle;
//	}
...
//			int max = bundle.getInt(INSTANCE_MAX, -1);
//			if (max > 0){
//				setMax(max);
//			}
//			int progress = bundle.getInt(INSTANCE_PROGRESS,-1);
//			if (progress > 0){
