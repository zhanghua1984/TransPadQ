...
public class DIDLLite
{
	...
	public String toxmlString() {
		try {
			...
			OutputStreamWriter writer = new OutputStreamWriter(e, "UTF-8");
			PrintWriter pr = new PrintWriter(writer);
			this.outputXml(pr);
			pr.flush();
			return e.toString();
	...
	public void outputXml(PrintWriter paramPrintWriter) {
		DIDLLiteNode localDIDLLiteNode = new DIDLLiteNode();
		String str = localDIDLLiteNode.getName();
		paramPrintWriter.print("<" + str);
		localDIDLLiteNode.outputAttributes(paramPrintWriter);
		paramPrintWriter.println(">");
		int i = this.getNContentNodes();

		for(int j = 0; j < i; ++j) {
			this.getContentNode(j).output(paramPrintWriter, 1, false);
		}
